...
from grask.capture_run import DEFAULT_EXCLUDE, plan
...
def test_excludes_grask_and_scratchpad_projects_by_default():
    result = plan(
        [
            transcript("-Users-me-projects-grask", "a"),
...
def test_a_docs_subdirectory_of_grask_is_still_grask():
    # The corpus really contains `-Users-...-projects-grask-docs-design-...`,
    # which is grask's own work under a name that does not end in "grask".
    result = plan(
        [transcript("-Users-me-projects-grask-docs-design-2026-07-21", "a")],
        exclude=DEFAULT_EXCLUDE,
        is_captured=NOTHING_CAPTURED,
        has_human_turns=ALL_HAVE_TURNS,
        limit=None,
...
def test_an_unparseable_transcript_is_dropped_not_raised(tmp_path: Path):
    from grask.capture_run import has_human_turns
