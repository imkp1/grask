...
Usage:
    uv run python -m grill.triage_run [--limit N] [--out PATH] [--workers N]
"""
...
from grill.survey import load_corpus
from grill.transcript import Session
from grill.triage import TriageVerdict, triage
...
def report(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "-" * 78,
        f"KEPT — {len(kept)} sessions grill would ask about",
        "-" * 78,
    ]
    ...
    lines += [
        "-" * 78,
        f"DROPPED — {len(dropped)} sessions grill would stay silent on",
        "-" * 78,
    ]
...
def main() -> None:
    ...
    # Under GRILL_HOME, never the working directory. This output is verbatim
    ...
    parser.add_argument("--out", type=Path, default=grill_home() / "triage-results.json")
