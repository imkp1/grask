...
sessions that end from now on are overwhelmingly grill's own. Waiting for the
queue to fill measures grill on grill. This walks the corpus already on disk
instead, skips grill's own project by default, and pays for a bounded number of
...
Usage:
    uv run python -m grill.capture_run [--limit N] [--go]
"""
...
from grill.transcript import extract, find_transcripts

DEFAULT_WORKERS = 6
DEFAULT_LIMIT = 40

# Any transcript whose project directory contains one of these is not corpus.
# "grill" keeps the tool from being evaluated on its own construction; the
# scratchpad dirs are agent side-sessions with no developer in them at all.
DEFAULT_EXCLUDE = ("grill", "scratchpad")
...
# hook's grill-only sessions) and under-quoted a 50-session run by 64%. Erring
...
def outcomes(store: Store, session_ids: Sequence[str]) -> str:
    ...
    if not probes["n"]:
        ...
        lines.append("  check grill.log before spending on a second batch.")
    return "\n".join(lines)
