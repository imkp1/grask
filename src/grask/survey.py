...
from grask.transcript import Session, extract, find_transcripts
...
def report(sessions: list[Session]) -> str:
    ...
    lines = [
        ...
        "",
        "Empty sessions are the triage floor: grask can never ask about these,",
...
def code_cost(sessions: list[Session]) -> str:
    ...
    underestimate in another: grask needs the session's *diff*, not the file, but
