...
**Scope:** v1, Claude Code plugin, local
**Revision:** v2 — audience changed from single-user to product; pull-based portal deleted.
See "Revision history" for what changed and why.
...
grill finds out which one it was, by asking questions you can't bluff.
...
- **v2 addition:** be used without willpower. A developer must encounter grill because they
...
**Trigger-based, attached to work the developer already does.** grill fires at the end of a
...
v1 rejection assumed a session-end grill costs 5–8 questions. It costs one.
...
Claude session ends
        │
        ▼
   Extractor ──▶ Topic ──▶ Core (grill) ──▶ Verdict ──▶ terminal
                              ▲                          │
   `grill <topic>` ───────────┘                          │
...
1. **`grill <topic>`** — CLI. Ships first, always works, needs no transcript. This is both
...
gaps on the user's behalf, and call it correct. **A grill that doesn't grill is worse than
...
grill was already doing this and not naming it. The object appeared twice under different
...
not just the prose here. Everything grill can honestly say after one probe is *"one
...
next probe: a recurring topic gets grilled on a claim it hasn't been grilled on yet.
...
inspectable: when a grilling feels off, inspect the rubric first.
...
distort — grill asks one thing:

```
was this worth asking?   [ y / n ]
```

**Yes-rate is the north-star metric.** If grill consistently earns a yes, almost everything
...
Claude Code `SessionEnd` hook. Reads the transcript, extracts one candidate, grills, exits.
Fails silently.
...
`grill` invocation — with no other change to this design.
...
Moments the user *engaged with*, classified by intent and ranked. Superseded in detail by
`2026-07-21-triage-intent-classifier-design.md`; the ranking below is the current one.
...
**If no candidate clears the bar, grill says nothing.** A session of boilerplate, config
...
concept on separate days, which is evidence of a different kind. See
`2026-07-21-triage-intent-classifier-design.md`.

### Attribution

Capture is greedy and does not attempt to determine whether the user learned something or
merely watched the agent do it. **The grilling resolves this.** If the user only watched,
...
topic's verdict sets the date. Probe selection then picks an ungrilled or previously hollow
...
grill does not name a model. The hook shells out to the user's already-authenticated Claude
...
- **Their quality bar is their choice.** A developer on the strongest model gets sharper
  probes; one who has downgraded for cost gets what they chose. grill does not get to make
...
| Failure | Behavior |
|---|---|
| LLM call fails mid-grill | Save transcript, mark session incomplete. Never lose user answers. |
...
**Step 1 is fed from real triage output, not hand-typed topics.** The `grill <topic>` CLI
...
**Terminal-only ceiling.** grill reaches Claude Code users and no one else. Judged correct
...
reframed toward topic-initiated grilling.
...
**Full 5–8 grill at every session end.** The version v1 correctly rejected. Priced for
