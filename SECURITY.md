...
grill reads your Claude Code transcripts and sends parts of them to a model. That is its
...
## What grill does with your data
...
- **Sends transcript content to a model** — your prompts, the agent's replies, and the
  ...
  already has.
- **Stores what it extracts locally**, in SQLite under `~/.claude/grill/` (or
  `$GRILL_HOME`), including verbatim quotes of things you typed.
- **Sends nothing anywhere else.** There is no telemetry, no analytics, no phone-home, and
  no second credential. grill has zero runtime dependencies, so there is no third-party
...
- Data written outside `GRILL_HOME` — into a working tree, a temp file that outlives the
  run, or any shared location.
- Transcript content reaching a destination other than the `claude` CLI.
- A path where grill executes content from a transcript rather than treating it as text.
  Transcripts are attacker-influenced input if you ever paste untrusted material into a
  session, and grill's prompts embed them.
- Anything that makes the `SessionEnd` hook block, crash visibly, or interfere with the
  session that spawned it.
- Any way to read another user's grill database through the tool.

Not vulnerabilities, by design: that grill reads all projects (documented above), that it
...
Expect an acknowledgement within a week. grill is a solo alpha project — there is no
on-call rotation and no SLA, and it is better to say that than to imply one.

When you report, **do not paste raw transcript content or `grill.log` output**. Describe
...
Two properties are worth knowing if you are assessing grill for use somewhere careful:

- **Model calls run with tools disallowed and slash commands disabled**
  (`NO_TOOLS`, `NO_SKILLS` in `src/grill/llm.py`). grill sends one self-contained prompt
  and expects one JSON object back; the model cannot read your repository during a grill
  call.
