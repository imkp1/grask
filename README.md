...
One probe, in the terminal. `/grask` inside Claude Code asks the same thing through the
native question UI.
...
Install as a *tool*, not with `uv sync`. Both surfaces below invoke grask by name, so it
has to resolve without a path; `uv sync` is for working on grask itself.
...
small, and stages 2 and 3 only run on the minority of sessions triage keeps. Measured
per-stage costs are in
[`docs/design.md`](docs/design.md#measured-cost-2026-07-20-107-session-corpus).
